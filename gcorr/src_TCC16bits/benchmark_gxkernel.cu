...
__global__ void GPU_Transpose_16bits(const cufftComplex *  PFB_FFT_out_Pol0, ComplexHalf * Transpose_out_data, int nr_channels, int samples_per_channel)
{
  ...
  Transpose_out_data[to_z * nr_channels * samples_per_channel + to_y * samples_per_channel + to_x].real = __float2half(tmp0[new_y][new_x].x) ;
  Transpose_out_data[to_z * nr_channels * samples_per_channel + to_y * samples_per_channel + to_x].imag = __float2half(tmp0[new_y][new_x].y) ;
}


__global__ void GPU_Reorder_after_transpose_16bits(const ComplexHalf*  Transpose_out_data, ComplexHalf* GPU_out_data)
{
...
int main(int argc, char *argv[]) {
  ...
  int samples_per_channel = arguments.nsamples / (arguments.nchannels * 2);
  int nr_baseline = (arguments.nantennas * (arguments.nantennas + 1) / 2);
  int n_blk_spectra = samples_per_channel/NR_TIMES_PER_BLOCK;
  ...
  ComplexHalf * samples_half1;
  ComplexHalf * samples_half2;
  //std::complex<__half> * samples;
  std::complex<float> * visibilities;
  cudaMalloc((void **)&samples_half1, arguments.nchannels * samples_per_channel * arguments.nantennas * npolarisations * sizeof(ComplexHalf));
  cudaMalloc((void **)&samples_half2, arguments.nchannels * samples_per_channel * arguments.nantennas * npolarisations * sizeof(ComplexHalf));

  checkCudaCall(cudaStreamCreate(&stream_tcc));
  //checkCudaCall(cudaMallocManaged(&samples, arguments.nchannels * samples_per_channel * arguments.nantennas * npolarisations * sizeof(ComplexHalf)));
  checkCudaCall(cudaMallocManaged(&visibilities, arguments.nchannels * nr_baseline * npolarisations * npolarisations * sizeof(std::complex<float>)));


  dim3 threads, blocks;
  threads.x = NR_TIMES_PER_BLOCK; threads.y = npolarisations ; threads.z = 1;
  blocks.x = n_blk_spectra; blocks.y = arguments.nchannels ; blocks.z =  arguments.nantennas; 

  dim3 threads_shared, blocks_shared;
  threads_shared.x = 32; threads_shared.y = 32; threads_shared.z = 1;
  blocks_shared.x = arguments.nchannels/threads_shared.x;
  blocks_shared.y = samples_per_channel/threads_shared.y; 
  blocks_shared.z = arguments.nantennas * npolarisations;
  ...
  int corrThreads, blockchan, nchunk, ccblock_width = 1024; //added by weiyu
  ...
  */
  ...
  for (i = 0; i < arguments.nloops; i++) {
    ...
    GPU_Transpose_16bits<<<blocks_shared, threads_shared>>>(channelisedData, samples_half1, arguments.nchannels, samples_per_channel);
    GPU_Reorder_after_transpose_16bits<<<blocks, threads>>>(samples_half1, samples_half2);
    timerEnd(&timers);
    
    timerStart(&timers, "TCC");
    tcc_correlator.launchAsync((CUstream) stream_tcc, (CUdeviceptr) visibilities, (CUdeviceptr) samples_half2);
  ...
  cuComplex *PFB_FFT_out_host; 
  cudaHostAlloc((void **)&PFB_FFT_out_host, (arguments.nantennas * npolarisations * arguments.nsamples * sizeof(cuComplex)), cudaHostAllocDefault);
  cudaMemcpyAsync(PFB_FFT_out_host, channelisedData, (arguments.nantennas * npolarisations * arguments.nsamples * sizeof(cuComplex)), cudaMemcpyDeviceToHost, stream_DtoH);
  cudaDeviceSynchronize();

/*
    for (int channel_i=0; channel_i < arguments.nantennas * npolarisations * arguments.nsamples; channel_i++)
    {
      if(PFB_FFT_out_host[channel_i].x!=0 || PFB_FFT_out_host[channel_i].y!=0)
      {
        std::cout << "channelisedData: " << PFB_FFT_out_host[channel_i].x << PFB_FFT_out_host[channel_i].y << std::endl;
      }
    }
    */
  ...
  checkCudaCall(cudaFree(samples_half1));
  checkCudaCall(cudaFree(samples_half2));
  cudaStreamDestroy(stream_DtoH);
  checkCudaCall(cudaStreamDestroy(stream_tcc));
  cudaFreeHost(PFB_FFT_out_host);
