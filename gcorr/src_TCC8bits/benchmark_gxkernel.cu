...
int main(int argc, char *argv[]) {
  ...
  tcc::Correlator correlator(NR_BITS, arguments.nantennas, arguments.nchannels, samples_per_channel, npolarisations, NR_RECEIVERS_PER_BLOCK);

  cudaStream_t stream_tcc;

  char2 * samples_char1;
  char2 * samples_char2;
  //std::complex<int8_t> * samples;
  std::complex<int32_t> * visibilities;
  cudaMalloc((void **)&samples_char1, arguments.nchannels * samples_per_channel * arguments.nantennas * npolarisations * sizeof(char2));
  cudaMalloc((void **)&samples_char2, arguments.nchannels * samples_per_channel * arguments.nantennas * npolarisations * sizeof(char2));
  
  checkCudaCall(cudaStreamCreate(&stream_tcc));
  //checkCudaCall(cudaMallocManaged(&samples, arguments.nchannels * samples_per_channel * arguments.nantennas * npolarisations * sizeof(std::complex<int8_t>)));
  checkCudaCall(cudaMallocManaged(&visibilities, arguments.nchannels * nr_baseline * npolarisations * npolarisations * sizeof(std::complex<int32_t>)));
  ...
  */
  ...
  timerAdd(&timers, "CrossCorrShared"); //added by weiyu
  ...
  for (i = 0; i < arguments.nloops; i++) {
    ...
    correlator.launchAsync((CUstream) stream_tcc, (CUdeviceptr) visibilities, (CUdeviceptr) samples_char2);
